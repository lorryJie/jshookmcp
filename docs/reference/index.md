...
| `antidebug` | 反反调试 | full | 反反调试域，集中提供检测与绕过浏览器端反调试脚本的工具。 |
| `apk-packer` | APK 加固识别 | full | 通过匹配 `lib/<abi>/lib*.so` 文件名识别 Android APK 加固层；框架不内置签名表，调用方通过 customSignatures 提供（ReDoS 防护正则编译）。不脱壳、不动态执行、不与加固载荷交互。 |
| `binary-instrument` | 二进制插桩 | full | 二进制插桩域，提供二进制分析和运行时插桩能力。 |
| `binary-secrets` | 二进制密钥 | full | 扫描二进制文件中硬编码的密钥候选（高熵原始数据、Base64、hex），纯只读信息输出。 |
...
| `canvas` | 画布引擎 | full | 游戏引擎 Canvas 逆向分析域，支持 Laya/Pixi/Phaser/Cocos/Unity 等主流游戏引擎的指纹识别、场景树导出和对象拾取。 |
| `coordination` | 协调 | full | 用于会话洞察记录与 MCP Task Handoff 的协调域，衔接大语言模型的规划与执行。 |
...
| `debugger` | 调试器 | workflow, full | 基于 CDP 的断点、单步、调用栈、watch 与调试会话管理域。 |
| `encoding` | 编码 | workflow, full | 二进制格式检测、编码转换、熵分析与 protobuf 原始解码。 |
| `evidence` | 证据图 | full | 逆向证据图域，用图结构串联 URL、脚本、函数、Hook 与捕获产物之间的溯源关系。 |
| `extension-registry` | 扩展注册 | full | 扩展注册域，管理和发现社区扩展。 |
| `graphql` | 图查询 | workflow, full | GraphQL 发现、提取、重放与 introspection 能力。 |
| `hooks` | 钩子 | full | AI Hook 生成、注入、数据导出，以及内置/自定义 preset 管理。 |
| `instrumentation` | 仪器化 | full | 统一仪器化会话域，将 Hook、拦截、Trace 与产物记录收束到可查询的 session 中。 |
| `macro` | 宏 | full | 子代理宏编排域，将多步工具调用组合为可复用的宏流程。 |
| `maintenance` | 维护 | workflow, full | 运维与维护域，覆盖缓存、token 预算、环境诊断、产物清理与扩展管理。 |
...
| `sandbox` | 沙盒 | full | 基于 QuickJS WASM 的安全沙箱域，支持执行自定义脚本并调用 MCP 工具。 |
| `shared-state-board` | 共享状态板 | workflow, full | 跨 Agent 状态同步域，提供全局共享的状态板用于多 Agent 协作。 |
| `skia-capture` | Skia 捕获 | workflow, full | Skia 渲染引擎捕获域，用于 UI 渲染分析和可视化。 |
...
| `workflow` | 工作流 | workflow, full | 复合工作流与脚本库域，是 built-in 高层编排入口。 |
